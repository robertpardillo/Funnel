...
# Defining port
port = int(re.findall('.*:([1-9]*)', url['model3D'])[0])
define("port", default=port, help="run on the given port", type=int)


class Session(object):
    def __init__(self,_id):
        self.socket_handler = socketClient.SocketHandler('ws://' + url['handler'] + '/model3D?id={}'.format(_id))
        self.socket_design = socketClient.SocketDesign('ws://' + url['design'] + '/model3D?id={}'.format(_id))
        self.socket_CFD = socketClient.SocketModel3D('ws://' + url['CFD'] + '/model3D?id={}'.format(_id))
        self.id = _id
...
app = tornado.web.Application([(r'/handler', SocketHandler), (r'/design', SocketDesign), (r'/CFD', SocketDesign),
